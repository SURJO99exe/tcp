...
uid=4425403128,password=JOBAYAR_CODX-Z6E3PXF1T

# OR Format 2: Line-separated
# uid: 4425403128
# password: JOBAYAR_CODX-Z6E3PXF1T
...
def send_tiktok_info(username):
    ...
    try:
        response = requests.get(
            f"https://-gamer-king.vercel.app/tt-info/tiktok?username={username}",
            timeout=15
        )
...
# NEW ATTACK COMMAND (AUTO STOP AFTER 1 SECOND)
                        if inPuTMsG.strip().startswith('/hexozentaerror '):
...
3🤫0🤫. Chat With AI [FF00FF]/ai [question]
    """
